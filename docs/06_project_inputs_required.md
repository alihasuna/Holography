...
Status: 2026-09-21. Each item is a quantity that no physics derivation and no repository file can
provide. Until it is supplied by the laboratory, the corresponding model parameter is an
...
2. Energy spread (FWHM, eV) and effective source size or measured spatial-coherence width at the specimen.
3. Illumination convergence semi-angle at the specimen (mrad) for holography and for REM imaging.
...
6. Detector: type, MTF or its published parameters, gain, dose per hologram (electrons/pixel), exposure time, drift during exposure.
...
10. Sign conventions: which tilt-coil direction moves the beam toward the surface, and which image axis is the beam direction (the foreshortened one).

## C. Sample

11. (blocking) Surface orientation actually used (Si(001) vs Si(111)), miscut angle and direction, typical terrace widths.
12. (blocking) Surface preparation state: native oxide, HF-last, UHV flash, ion-milling parameters, expected amorphous damage-layer thickness, any annealing. At 20 mrad glancing incidence a 1 nm amorphous overlayer is traversed over about 50 nm of path, so this dominates the reflected amplitude and must be modelled rather than ignored.
13. Pattern geometry for the patterned samples: lateral dimensions, nominal step or trench heights (nm) from AFM or SEM, edge profile, orientation relative to the beam.
14. Whether the intended observable is atomic steps (heights 1.36 A, 2.72 A or 3.14 A) or nanometre-scale patterned steps. The two regimes need different quantification strategies because the height wrap period is only 0.4 to 1.5 A.
...
17. Residual phase of an empty hologram (curvature, distortion) and whether a reference hologram is recorded with every object hologram.
...
## F. Material parameters to be sourced (not measured by the lab, but not yet sourced here)

20. Silicon mean inner potential `V0` with a citable measured or calculated value and uncertainty. The value 12.0 V used in this repository is an ASSUMPTION; a change of 1 V shifts the single-bilayer step phase at the (444) condition by about 0.34 rad (0.05 A in height).
