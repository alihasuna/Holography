...
Status: revision 2, 2026-09-21 (corrected after `docs/agent_reports/E_review.md`), written during the
...
| A2 | A free-standing plate (vacuum above and below) with periodic boundaries along the surface normal represents a semi-infinite crystal surface. | ASSUMPTION | Not supportable with the default cell: 81 percent of the incident wave enters the end face, only 3 percent reflects from the top surface, and the Laue-transmitted (n,-n,n) beam is collinear with the specular beam (C sections 5.2 and 5.3). |
...
| B9 | Shadowed regions are excluded from quantification. | DERIVED_HERE | A step transverse to the beam shadows `h/tan(theta_ext)` of surface (139 A per bilayer, 444 nm for a 10 nm mesa at 22.5 mrad); the model must ray-trace visibility. |
