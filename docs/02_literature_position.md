...
* Every record of `docs/references.bib` was checked against Crossref, DataCite, arXiv, a patent record
  or the project's own page (B3; counts printed by `tools/bib/crossref_check.py report`).
...
* Read in full: P04 (text and captions), P07, the patent US 4,998,788 (PAT01), the erratum P02E, the P49
  preprint and the Si mean-inner-potential preprint arXiv:2607.05948v1. Read in part: US 5,192,867
  (cols. 3-4 and part of col. 7), the prismatique documentation (API, literature and licence pages;
  examples and STEM pages not read), the Prismatic pages listed in L2 C.1, the P49 code (README and
  output routine), the C03 accepted manuscript (sec. 3.4, 3.5, 5, 6.2, 9.1; L5 2.5) and Hÿtch et al.
  2010, the companion of P31 (sec. 1-2; L5 2.6).
* Read at abstract level only: P01 (IOP page), P05, P06, P08, P09, P31 (PubMed), and the post-1993
  reflection papers listed below. P02 and P03 are known from their metadata only.
* Closed here (publisher pages or, where these were unreadable, the indices; L5 section 1): the bodies
  of P01, P02, P03, P06, P08, P09, P31 and all book chapters. P05 is open access (CC BY 4.0) but ScienceDirect, eScholarship and TARA block
...
* Not searched: Scopus, Web of Science, Google Scholar, and the Japanese databases (CiNii, J-STAGE
  full text); five OpenAlex topic queries and one Semantic Scholar relevance query failed on rate
  limits (L4 section 4.5), so OpenAlex was used only for the citation graph.
...
| P08 | Osakabe, Microsc. Res. Tech. 20(4), 457-462 (1992) | `+ABSTRACT(PubMed)`, sentences 2-3 and the last sentence: the phase shift of a Bragg-reflected wave measured by holographic interferometry with a field-emission gun and a biprism; geometrical path differences measured in units of the wavelength; a monatomic-step phase and a dislocation displacement field observed. |
...
| other Osakabe-era records | Osakabe et al., Proc. EMSA 47, 536 (1989) (abstract only, Crossref record); Osakabe et al., Ultramicroscopy 48, 483 (1993) and Osakabe, Surf. Sci. 298, 345 (1993) (titles only); Takeguchi, Harada and Shimizu, J. Electron Microsc. (1990), reflection holography of GaAs(110) with numerical reconstruction (abstract only, OpenAlex record; Crossref lacks authors, volume and pages) | METADATA_VERIFIED where Crossref is complete (L4 section 6.2); Takeguchi 1990 is UNVERIFIED in references.bib. The 1987 Optik supplement cited by the patent is unidentified. |
...
Scholar and OpenCitations COCI: 60 records, 57 distinct works; of the 60 records, 32 were classified
from an abstract and 28 from the title only (L4 sections 2 and 3). Result:

* Reflection electron holography was performed after 1993 by at least one group, the only one found here: the Yagi and
  Tanishiro group at the Tokyo Institute of Technology, in 2001 to 2003, on clean Si(111)7x7:
  Suzuki et al., Jpn. J. Appl. Phys. 40, 2527 (2001) (abstract only, Crossref record: energy-filtered interferometry in
...
* No peer-reviewed electron reflection-mode ptychography was found (Semantic Scholar returned zero hits
  ...
  L4 section 4.2). Every electron-ptychography abstract read that states a geometry is in transmission
  (one, Shpiro et al. 2025, states none), including the 20 keV SEM
...
* X-ray, EUV and visible reflection ptychography exists and is the analogue: Zhu et al. 2015 (Pt(111)
  ...
  (L4 section 4.3). Their forward models were not read. Report B (index level) found no abstract
...
* Senhorst, Witte and Coene, Opt. Express 34, 22163 (2026) (abstract only, OpenAlex record) show that the two-dimensional
...
* Dark-field holography in transmission: Hÿtch et al. 2010 (J. Phys.: Conf. Ser. 241, 012027; read): four
  ...
  repository's step phase (DERIVED_HERE, L5 2.6: a consistency check of convention, not a validation). P31 (Ultramicroscopy 111, 1328, 2011) is read at abstract level only, so its
...
* Biprism optics: P07 (read) describes single-, double-biprism and split-illumination optics
  ...
  optical-path phase, and says nothing on Osakabe's arrangement. PAT01 embodiment 2 is read here as a published
  reflection-mode split-illumination arrangement with a condenser-side biprism (DERIVED_HERE, L1 I7).
...
P04 (read) prints the propagator `exp(-i pi lambda |q|^2 t)` (Eq. (3); paraxial by inference, L2 A-I1) and an anti-aliasing
...
arXiv:2607.05948v1 (Schowalter, Kruse and Rosenauer; bulk-terminated slab, whose (110) faces are
DERIVED_HERE from the vacuum direction, L5 4; no uncertainty stated).
The abstracts of Pennington et al. 2015 and Auslender et al. 2024 report a surface dependence of the mean
inner potential. Candidate measured values (Kruse et al. 2006, DFT and holography; Wang et al. 1997, Si
nanospheres; Gajdardziska-Josifovska et al. 1993, crystal wedges, Si not confirmed) are closed. `V0 = 12.0 V` stays an ASSUMPTION (`docs/model_assumptions.md` B1).
...
OpenAlex API key to rerun the five OpenAlex topic queries.
