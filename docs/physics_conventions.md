...
Status: initial version, 2026-09-21. Every formula in this repository must obey these conventions
or state explicitly where it deviates. (Required by instruction file section 9.8; the same
conventions are used in `docs/agent_reports/C_physics_derivations.md` and
`tools/reflection_step_phase_calculator.py`.)
...
| Wavelength | Relativistic vacuum wavelength `lambda = h c / sqrt(T (T + 2 m_e c^2))`, constants from CODATA 2018 (`m_e c^2 = 510998.950 eV`; `h`, `c`, `e` exact SI-2019 values). 200 keV: 0.02507934 A. |
| Energies | Beam energy in keV; potentials in V; potential energy of the electron inside the crystal is `-e V0 < 0` for a positive mean inner potential `V0`. Charge `e > 0`. |
...
* Vacuum Bragg condition of order `n` along the (111) rod: `sin(theta_B) = n lambda / (2 d_111)`.
* Refraction (conservation of the surface-parallel wavevector; relativistic):
  `sin^2(theta_int) = (sin^2(theta_ext) + Delta) / (1 + Delta)`, with
  `Delta = (k_int^2 - k_ext^2)/k_ext^2 = V0 (1 + T/(m_e c^2)) / (T (1 + T/(2 m_e c^2)))`,
  `T` the kinetic energy in eV, `V0` in V. Critical angle `sin(theta_c) = sqrt(Delta/(1+Delta))`.
  At 200 keV and `V0 = 12 V` (ASSUMPTION), `Delta = 6.98e-5`, `theta_c = 8.356 mrad`.
  The non-relativistic form `Delta = V0/T` underestimates `Delta` by 16 percent at 200 keV; do not use it.
...
| Energies | keV (beam), eV (spreads), V (potentials) |
