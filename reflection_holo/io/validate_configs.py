...
Usage: venv/bin/python -m reflection_holo.io.validate_configs configs/cfg_*.yaml

For each file, prints the run-level verdict (PASS, or the refusal with the docs/06 item numbers or
the UNVERIFIED fields) and the placeholder-level summary. Writes a manifest under outputs/.
...
def main(argv: list[str]) -> int:
    ...
    out = write_manifest(manifest, outputs_root=repository_root() / "outputs")
